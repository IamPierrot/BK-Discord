     ...
     try {
          ...
          if (message.attachments.size > 0) {
               ...
               if (attachment && attachment.contentType) {
                   await saveAttachment(message.id, attachment.url, attachment.contentType);
               }
           }
          ...
          if (commandObject?.adminOnly && !message.member.permissions.has(["Administrator"])) return message.reply("Bạn không có quyền dùng lệnh này!");
